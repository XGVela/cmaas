...
BUILDER_NAME="config-service-builder"
BUILDER_VERSION="v0.1"

MICROSERVICE_NAME="config-service"
...
docker build --rm $BUILDER_ARG -f ./build/config-service-builder-dockerfile -t $BUILDER_NAME:$BUILDER_VERSION .

##NANO SEC timestamp
BUILDER_LABEL="config-service-builder-$(date +%s%9N)"
echo -e "\e[1;32;40m[CMAAS-BUILD] Build MICROSERVICE_NAME:$MICROSERVICE_NAME, Version:$MICROSERVICE_VERSION \e[0m"
docker build --rm --build-arg BUILDER_LABEL=$BUILDER_LABEL -f ./build/config-service-dockerfile -t $MICROSERVICE_NAME:$MICROSERVICE_VERSION .
...
echo -e "\e[1;32;40m[CMAAS-BUILD] Upating config-service chart \e[0m"
cp -r ./charts/config-service $ARTIFACTS_PATH/charts/.
sed -i "s/cmaas_tag/$1/" $ARTIFACTS_PATH/charts/config-service/values.yaml
#sed -i "s/cim_tag/$4/" $ARTIFACTS_PATH/charts/config-service/values.yaml
